...
$query = "SELECT event_id,event_title,event_location,event_venue_lat,event_venue_long,event_discipline,event_small_logo,event_url, event_brand, DATE_FORMAT(event_date,'%d %M %Y') AS event_date_format FROM events WHERE event_live='Y' AND event_listings='Y' AND event_closed = 'No' AND event_date >=DATE(NOW()) ORDER BY event_date DESC";
...
while ($row = @mysqli_fetch_assoc($result)){
  ...
  echo 'event_date_format="' . parseToXML($row['event_date_format']) . '" ';
